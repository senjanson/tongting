...
一个 Chrome MV3 扩展：在 YouTube 网页上边看边生成目标语言（默认简体中文）的翻译字幕，并可选择播放翻译配音。文本翻译走**你自己**部署的 [sub2api](https://github.com/Wei-Shaw/sub2api) 服务——扩展不内置任何 Key，也没有托管后端。没有可读字幕的视频需要另行配置语音识别服务，本仓库附带了一个本地服务。
...
在侧栏「搜索」里用中文说出你想看什么，AI 生成 3 组英文搜索词和中文注释，每组都可编辑、复制、在新标签页搜索。最近记录只留在本机。复用你已经配置好的翻译服务和模型。
...
1. 打开扩展设置页（工具栏弹窗 → **设置**）。
2. 填写 **sub2api 服务地址（Base URL）** 与 **API Key**。只支持 https，本机调试可用 `http://127.0.0.1:<端口>`。
3. 「记住在本机」默认勾选：Key 保存在本机，重新加载扩展或重启浏览器后仍在。取消勾选则只存在会话存储里。
4. 点击**获取模型列表**选择模型，再点**检查连接**——它会依次验证主机权限、可达性、认证、模型列表、模型本身、小规模翻译与流式返回。

扩展只申请你填写的那一个 origin 的主机权限，不会发往别处。

语音识别与语音合成是独立能力，需要分别配置、分别检查。sub2api 的音频检查会产生一次真实计费调用，只有你明确勾选后才会执行。
...
- **只授权一个 origin。** 主机权限只申请你填写的那个 sub2api 地址。
