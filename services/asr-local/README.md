...
- `durationMs`：1–30000 的整数，默认 20000。接近媒体结尾时自动缩短，允许不足一秒的窗口填补跳转后的时间空隙。
...
媒体结尾优先取所选音轨签名地址中的 `dur`（YouTube 的 approxDurationMs，精确到毫秒）；缺少或无效时才退回 yt-dlp 的整秒时长，后者可能比播放器的时长短将近一秒。`startMs` 到达或超过结尾时返回 `416 youtube_range_unavailable`，并带响应头 `X-Tongting-Media-End-Ms`，值为可读音频的实际结尾（毫秒）。音轨可能比标称时长略短：结尾前 2 秒内解码不到任何音频时同样返回 416，此时该头的值等于请求的 `startMs`；离结尾更远的空结果仍按读取失败返回 `502 youtube_audio_failed`。客户端应把 `startMs` 不小于该值的范围视为已读到结尾，而不是错误。旧版服务不带此头。
...
仅解析 `https://www.youtube.com/watch?v=<ID>`，媒体来源及重定向仅接受 HTTPS 的 `*.googlevideo.com` 音轨。优先选择有索引的 M4A 音轨。单独子进程通过 256 KiB 的有界 HTTP Range 请求读取媒体，并在随机回环端口提供随机入口，供 FFmpeg 按请求时间精确截取为 16 kHz、单声道、16-bit PCM；每个片段网络读取最多 16 MiB。这避免旧版 FFmpeg 的无界请求被限速。音频使用内存管道，不写临时音频文件。最多缓存 8 个媒体地址、每个 120 秒，不缓存识别全文。本次请求新解析的地址在该请求失败或被取消时清理；之前已缓存的地址在请求被取消（例如跳转）时保留，在解码失败、未取得音频或取段超时时清理，下次请求重新解析；到达结尾的 416 不清理地址；服务停止时清空全部地址。取段整体最多等待 60 秒；超时、客户端断开或服务停止会杀死独立进程组及其 JavaScript/FFmpeg 子进程，并等待退出。下载器和 FFmpeg 的原始诊断不会回显到 API 或日志。
...
默认测试不会访问 YouTube 或下载模型。`tests/test_youtube.py` 覆盖参数、鉴权、范围与媒体结尾、媒体来源、缓存保留与清理、异常、取段取消、进程创建竞态与停止；真实模型测试依旧需显式启用。测试服务使用随机回环端口，不需要停止正在使用的 8765 服务。
