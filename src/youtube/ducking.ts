 ...
 * - ducking 期间 volumechange 读到的值不是自己设置的：视为用户新意图，更新基准并停止本次调整；
...
export function createDuckController(): DuckController {
  ...
  return {
    ...
    duck(target, level) {
      ...
      if (!active || applied === undefined || Math.abs(now - applied) > EPSILON) {
        // 新一轮 ducking，或用户在上一次调整后改过音量：以当前值为用户基准。
        base = now;
      }
      try {
        overridden = false;
        target.volume = clamp01((base ?? now) * clamp01(level));
