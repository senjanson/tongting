...
**功能代码已完成（P0–P9），自动化测试通过；真实 sub2api、真实 YouTube 页面、真人听感与长时间运行尚未验收。**
...
- **验证方式**：
  ...
  - Playwright E2E 使用本地 YouTube 夹具页面、模拟 sub2api、真实 Chromium 扩展与真实本地识别服务。
  - 结果与覆盖范围见 [docs/VALIDATION.md](docs/VALIDATION.md)。
- **尚未实测**：
  - 本机无法访问 www.youtube.com，真实页面结构、字幕轨道读取、广告等只在夹具上验证过。
  - 没有提供 sub2api Base URL 与 Key，文本、流式、语音识别、语音合成等服务能力均为「未验证」。
...
- 无字幕视频：在用户点击后捕获当前标签页音频，交给已配置的语音识别服务，再进入同一条翻译管线。
- 翻译配音：系统语音（或经实测可用的 sub2api 语音合成），可调声音、语速、配音音量与配音时降低原声。
- 目标语言切换、暂停/继续翻译、停止并释放音频、跳转、换视频、倍速下的正确同步与清理。
- 字幕记录与工作台：搜索、收藏、带时间的笔记、复制，导出 SRT / VTT / TXT，并如实显示覆盖范围。
- 设置与连接检查：Base URL、API Key（默认只保存在浏览器会话）、按单一 origin 授权、协议自动检测、模型发现与手动模型 ID、分项连接检查。
