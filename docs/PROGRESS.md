...
### 2026-09-24：四主题外观系统与侧栏翻译页重排

- 依据：Claude Design 画布「Vocasub UI 重设计」的「多主题」页（纸墨 / 夜墨 / 影院 / 声波）。
- 完成：
  - 新设置 `uiTheme`（`auto` / `paper` / `ink` / `cinema` / `wave`，默认 `auto`：浅色纸墨、深色夜墨），不进入翻译指纹。
  - `tokens.css` 按 `<html data-tt-theme>` 切换四套变量；快照到达前先用本机上次的主题，避免闪屏。演示模式沿用用户保存的主题。
  - 西文字体（Figtree、IBM Plex Sans/Mono、Bricolage Grotesque，SIL OFL）只打包基本拉丁子集，约 122 KB，不访问字体服务；中文用系统字体。
  - 侧栏「翻译」页重排：当前字幕卡片（按播放时间显示当前句译文与原文、分段进度）、语言对行、输出方式、可展开的设置行（翻译风格 / 播放方式 / 字幕样式 / 声音）、主题选择、固定底栏（主按钮 + 停止）。原有功能与提示全部保留。
  - 共享组件、侧栏其他标签、设置页（常规区新增主题选择）、工具栏弹窗、字幕工作台改为卡片式风格。
  - 播放器字幕层按主题切换外观（`display/settings` 带 `uiTheme`，切换时重发，不重启会话）。
- 验证：类型检查、lint、格式通过；单元 985 项、集成 246 项通过；全量 E2E 58 项通过、18 项跳过（两轮并发运行造成的 5 项文件冲突失败，单独重跑全部通过）；新增 `ui-theme` E2E；README 截图重新生成，含四主题侧栏与字幕层对比。
- 未验证：真实 YouTube 页面上的字幕层观感（截图来自本地夹具页）；Windows / Linux 的中文字体回落效果；真实侧栏容器（E2E 以普通标签页打开侧栏页面）。

### 2026-09-25：同步优先读不到完整字幕时退回边播边译

- 起因：用户反馈同一视频在另一台电脑上完全不可用。同步优先读不到完整字幕轨道时，没有配置本地识别会报「同步优先需要启用音频预读的本地识别服务」；只配置了 sub2api 识别也会报错，且用户的 sub2api 识别接口返回 404。
- 完成：
  - 读不到完整轨道、又不能预读音频时，本次自动改为边播边译（读取播放器已显示的字幕）；完全没有字幕时改为边播边识别。两种情况都会提示，括号里写明读不到的具体原因（超时 / 格式无法识别 / 播放器不可用 / 桥未就绪 / 找不到轨道）。
  - 自测发现并修复：准备阶段闸门已暂停视频，退回后视频一直停着。现在同一会话退回边播边译或边播边识别时，闸门把自己暂停的视频恢复播放。用户暂停、切换「连续播放」、暂停或停止翻译时，仍不会自动播放。
- 验证：
  - 类型检查、lint、格式检查通过；单元 993 项、集成 247 项通过；全量 E2E 58 项通过、18 项门控跳过。新增闸门「交还播放」单元测试、内容脚本退回回归测试，并把夹具 E2E 改为边播放边开始翻译，由扩展自动恢复播放。
  - 真实 YouTube（隔离 Chromium、E2E 构建、本地模拟翻译服务）：默认设置与「识别选 sub2api」两种配置都不再报错，约 5.7 秒后退回边播边译，提示原因为「播放器没有返回可读取的字幕内容」。修复前视频停在准备阶段的位置，修复后约 6 秒后自动继续播放。
- 仍未验证：
  - 在这个隔离 Chromium 里，YouTube 对播放器自己的字幕请求（带 pot）也返回 200 空内容，画面上没有 YouTube 字幕，所以「显示字幕 → 翻译」只由夹具 E2E 覆盖。
  - 另一台电脑读不到完整轨道的根本原因尚未确认。

### 2026-09-25：诊断日志

- 起因：v0.1.4 装到另一台电脑后仍然读不到字幕、没有声音，但扩展没有任何运行记录，无法远程判断原因。用户要求添加日志功能（不记录 Key）。
- 完成：
  - worker 端日志存储：环形缓冲（约 1500 条、600 KB 上限），3 秒合并写入 `chrome.storage.local`，worker 即将休眠时立即写入。
  - 记录内容：会话创建（非敏感设置摘要）、会话与页面状态变化（阶段、字幕来源、提示、错误码、缓冲、资源状态，只记变化字段）、翻译进度里程碑、首次发声、完整轨道读取失败的错误码、配音错误、原声音量调整；页面端记录播放器每次 timedtext 响应的状态码与正文长度（含空正文）、桥的读取命令与结果、兜底请求结果、读取当前显示字幕的开关与条数、播放器 CC 按钮状态、原声音量实际设置、缓冲闸门状态。
  - 页面记录经内容端口批量上报，不为日志唤醒 worker；worker 端再次校验、脱敏，并按标签页限速（每分钟 240 条）。
  - 脱敏：敏感字段名的字符串值、URL 参数值、sk- 密钥、Bearer 令牌、40 位以上令牌串一律替换；不记录字幕原文、标题或音频。导出的设置中服务地址只保留 origin，术语表只给条数。
  - 设置页新增「诊断日志」：复制、下载（.txt）、清空。YouTube 页面控制台同时输出 `[vocasub]` 记录。
- 验证：
  - 新增单元测试（脱敏、存储上限与持久化、页面记录校验与限速、状态变化记录、内容脚本暂存发送、导出排序与设置摘要、桥记录空正文且不含 URL）与集成测试（会话日志、页面上报、导出不含 Key、清空、无存储时导出）。
  - 真实 YouTube（隔离 Chromium、E2E 构建、模拟翻译服务、输出「字幕 + 配音」）：在设置页点击「下载日志」得到的文件记录了播放器两次 timedtext 响应 200 且正文长度 0、兜底请求同样为 0、读取失败码 captions-load-timeout、退回边播边译、CC 按钮已开、原声音量被设为 0；文件中没有测试 Key、pot / signature 参数值或字幕原文。
  - 类型检查、lint、格式检查通过；单元 1006 项、集成 251 项通过；生产与 E2E 构建通过。
- 未完成的验证：本轮全量 E2E 未能在正常环境下跑完。电脑反复休眠、锁屏期间，缓冲播放与工具栏弹窗用例失败（夹具视频不前进、窗口无法聚焦）；用未修改的上一提交构建重跑同样的用例，以相同方式失败，判断为环境原因。发版前需在电脑唤醒时重跑全量 E2E。
- 已知问题（未修改）：「字幕 + 配音」且「原声处理」为「全程静音」时，没有译文可朗读也会把原声调为 0，视频完全没有声音。

### 2026-09-26：诊断日志覆盖面实测与补齐

- 用夹具页面 + 模拟服务逐个制造 11 种故障，导出日志检查能否看出原因：正常翻译、YouTube 返回空字幕、无字幕且未配置识别、Key 无效 401、模型不存在 404、限流 429、服务出错 500、服务无响应超时、原声全程静音、未授予服务地址权限、sub2api 语音识别接口 404（检查连接）。
- 首轮发现并补齐：
  - 翻译请求失败没有记录 HTTP 状态与错误码（500、超时只看到笼统提示）：调度器与会话分别记录请求失败与字幕失败，同类失败只在变化时和每 20 次记录；熔断信息附上最后一次的 HTTP 状态。
  - 「检查连接」结果与界面操作失败没有记录：新增 `connection.check`（每项状态、原因码、说明）与 `ui.command-failed`（只记命令种类，不记参数）。
  - 页面记录 1 秒批量发送，刚发生就导出会缺：改为 0.3 秒；MAIN world 桥在内容脚本就绪前的记录暂存，收到内容脚本第一条消息后补发；从缓存重放字幕时也记录。
- 补齐后 11 种故障都能从日志直接看出原因；真实 YouTube 重跑通过，日志中没有测试 Key、pot / signature 参数值或字幕原文。
- 日志仍看不到的：YouTube 以外的网络环境（代理、DNS）只能从错误码间接判断；系统语音列表与实际听感；界面渲染问题（未记录界面异常）；扩展没有注入到页面时（页面在安装前打开）只能看到「没有页面」。
- 验证（2026-09-26）：`pnpm typecheck`、`pnpm lint`、`pnpm format:check` 通过；单元 1007 项、集成 253 项全部通过；`wxt build` 与 E2E 构建通过；全量 E2E 58 项通过、18 项跳过（均为需显式开启的真实音频、系统配音、语音识别、付费服务与 README 截图用例）。

### 2026-09-26：外部审查 18 条核查与服务回显 Key 的脱敏

- 起因：外部审查提交 18 条问题（1 条 P1、14 条 P2、3 条 P3）。逐条读代码并用临时用例复现（模拟对象，未在真实浏览器与服务上复现）：16 条成立，2 条部分成立（流式认证错误有熔断兜底；offscreen 关闭窗口内的新捕获是失败而非成功），没有不成立的；多数 P2 实际触发概率低，建议降为 P3/P4。
- 核查结论与计划：
  - 本版（0.1.5）修复：服务在错误正文里回显不带 `sk-` 前缀的 Key 时，完整 Key 进入错误详情、会话快照与诊断日志导出（P2，已复现）。
  - 计划 0.1.6 修复：工作台未恢复的笔记草稿在切换视频或窗口重新聚焦时被删除（P1）；换成有效模型后旧的 config 错误不清除、缓冲一直 blocked 且「重试」无效（P2）；视频自然结束时实时识别尾段丢失（P2）；合并后超过 90 秒的长字幕被调度窗口漏掉、同步优先卡住（P2）；`{"events":[]}` 等空轨道被当作完整轨道成功，不回退（降为 P3）。
  - 可顺手修（P3/P4，小改动）：保存识别令牌期间启动会话用旧令牌、流式认证错误按服务故障重试、非模型 200 响应让检查判定 Key/模型有效、检查后 40 ms 内改 ASR/TTS 不清旧报告、清空日志写入失败仍报成功、配音 `tts/play` 超时后未按 remoteId 停止、端口失效瞬间切轨丢失、VTT 相接同文 cue 丢时长、快速回选原主题丢最后一次点击、长分段配置下识别积压超过 30 秒。
  - 不完整修：超过 2 万条被截断的轨道（现实中需 11 小时以上密集人工字幕），只修正提示文案；offscreen 关闭期间接受新任务（跨标签页、毫秒级窗口），延后。
  - 额外发现：截断轨道在同步优先下的提示声称在读取画面字幕、原因写成超时，均不符实际；VTT 滚动去重会删掉人工 VTT 中合法重复的首行。
- 完成（Key 脱敏）：
  - 新增 `src/domain/known-secrets.ts`：在 worker（加载与保存 Key、识别令牌时）和 offscreen（识别路由、云端配音请求）登记凭证原文，最多保留 8 个，换 Key 后旧值仍脱敏。
  - `redactSecrets`（错误详情、流式错误、识别与配音错误码）先按原文替换已登记凭证，并新增通用规则：24 位以上且同时含字母与数字的连续串替换为 `[REDACTED_KEY]`。
  - 诊断日志 `redactString` 在截断前按原文替换已登记凭证；导出时再次脱敏，早先写入的记录也会被替换。诊断日志不启用 24 位通用规则，避免遮住会话 ID。
- 验证：新增单元测试（32 位十六进制 Key 回显、不误伤模型名/视频 ID/请求 ID/会话 ID、sanitizeDetail 路径、已登记任意格式 Key 在错误文本与日志中替换、先替换后截断、登记上限）与集成测试（设置含标点的 Key 后，翻译失败与页面记录中的 Key 不进入日志存储与导出）。把新测试放到未修改的上一提交上运行，5 个针对泄漏的用例全部失败，修改后通过。

### 2026-09-26：外部审查 P1/P2 修复（计划 0.1.6）

- 工作台笔记草稿（P1）：草稿存储改为每个视频可有多份（带写入者 id，兼容旧的单份格式）；删除必须指明内容或写入者，不再按视频整体删除。未处理的旧草稿在切换视频、窗口重新聚焦或其他页面保存触发的重读、另行编辑并保存、选择「载入最新版本」后都保留；恢复并保存成功或点「丢弃」才删除。
- 换成有效模型后仍卡住（P2）：会话记录由翻译失败写入的阻断错误；换模型（译文配置变化）、换 Key 等调度设置变化时按来源清除，之后任一字幕翻译成功也清除（含 config 类）。识别、捕获等其他来源的错误不受影响。
- 长字幕漏调度（P2）：normalize 合并同文重复段时不超过 60 秒单条上限，超出另起一条；调度器回看量改为 max(60 秒, 当前字幕的实际最长跨度)，只在字幕顺序变化时重算。
- 空字幕当作完整轨道（P2→P3）：读取到 0 条有效字幕时按 `captions-empty` 读取失败处理，进入现有回退（可见字幕 / 预读 / 识别），提示原因「播放器返回的字幕轨道没有内容」；运行中切到空轨道时保留当前来源与译文，记录 `captions.track-empty`。
- 视频结束丢识别尾段（P2）：实时识别下视频自然结束时，先发结束锚点（结束后的音频丢弃），再请求 offscreen `capture/drain`：收尾分段器缓冲、等锚点确认与识别队列排空（上限 12 秒）；之后等结尾 30 秒内的字幕翻译完（上限 8 秒），再以 `video-ended` 结束。排空期间重播或跳转会取消结束，停止会话立即结束排空。播放锚点在 `ended` 时视为暂停。新增诊断记录 `asr.end-drain`。
- 验证：新增单元测试（草稿存储、识别队列 whenIdle、捕获收尾的正常/超时/停止、normalize 合并上限、调度器长字幕回看）与集成/组件测试（草稿在各种切换与重读后保留、换模型与换 Key 清除错误、空轨道回退与运行中空轨道、视频结束先收尾再停止、收尾中重播、翻译上限）。把新测试放到未修改的代码上运行均失败，修改后通过。
- 真实浏览器对照（Chromium + E2E 构建 + 夹具 YouTube + 模拟 sub2api；E 用真实 tabCapture 与本地 faster-whisper small）：同一脚本分别对未修复构建（bb45d5d）与修复后构建运行。
  - A 模型不存在后改为有效模型：未修复仍显示 model-not-found、缓冲 blocked；修复后错误清除、缓冲 ready（视频保持暂停，需手动播放，闸门「受阻只暂停一次」的原有设计）。
  - B 同文字幕合并超过 90 秒、从 95 秒开始同步优先：未修复一直 preparing、翻译请求 0 次；修复后 ready。
  - C 字幕轨道正文没有有效字幕：未修复当作 full-track、无提示；修复后退回读取画面字幕并提示「播放器返回的字幕轨道没有内容」。
  - D 工作台旧草稿，另写笔记保存后：未修复草稿被删、刷新后不再提示；修复后草稿保留并继续提示。
  - E 语音持续到视频结尾（约 37 秒）：未修复字幕记录缺最后一句；修复后最后一句完整识别并翻译。
- 未验证：真实 YouTube 页面与真实 sub2api 上的上述场景。
