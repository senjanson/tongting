...
**最新补充：同步优先缓冲已实现。** 默认准备 10 秒译文（可选 5 / 10 / 20 秒），支持完整字幕提前翻译，以及公开录播音轨预读后的本地识别。真实 20 秒片段的预读识别约 6.25–9.11 秒，其中一段再经真实 Luna 翻译用了 5.597 秒；新增浏览器缓冲播放 4 项通过，前端单元/集成 904 项、后端 216 项通过。当前本机服务已启用预读；20 分钟真实站点持续播放和真人试听仍未验收。详情与本轮限制见 [缓冲播放记录](reviews/2026-09-17/BUFFERED_PLAYBACK.md)。下表保留早期阶段记录；服务凭证、网络与短链路的后续进展以最新验收报告为准。
...
- 已处理本轮 [REVIEW.md](reviews/2026-09-17/REVIEW.md) 中的全部 17 项问题；修复映射、并发／失败／恢复矩阵和界面对照见 [FIXES.md](reviews/2026-09-17/FIXES.md)。
...
- 69 文件／818 项单元集成通过；当前常规浏览器全量 37 通过／14 门控跳过；另行真实服务两项、真实 YouTube 配音短探针一项通过。TypeScript、ESLint、Prettier、diff 检查与生产构建／打包通过。详细范围见 [LIVE_TEST.md](reviews/2026-09-17/LIVE_TEST.md)。
...
- 说明、验证范围及实拍截图见 [POPUP_LAYOUT.md](reviews/2026-09-17/POPUP_LAYOUT.md)。用户 Chrome 需要重新加载扩展一次；未访问用户浏览器配置或使用 Key，修改保持未暂存。
