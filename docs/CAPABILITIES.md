...
最后更新：2026-09-17。修复与本轮验证见 FIXES.md。

新增同步优先缓冲：完整字幕预翻译和无字幕公开录播音轨预读已接入。真实 YouTube 音轨 → 本地 Whisper → Luna 中文译文已通过短片段实测；暂停/恢复/快速跳转由隔离浏览器测试验证，真实站点长期播放仍未验收。缓冲表示译文就绪，不包含提前合成完整配音音轨。范围和耗时见 BUFFERED_PLAYBACK.md。

## 1. sub2api 服务（用户实例）

已使用用户提供的服务与 Key 实测，默认型号为 `gpt-5.6-luna`。能力仅限本次账号、模型与日期；详细结果见 LIVE_TEST.md。
...
真实 MIT 视频的短探针已通过：本地识别 → Luna → 当前可见中文字幕 → 实际系统中文朗读，4 条译文、3 句正常朗读结束，停止后无捕获或继续朗读。首句朗读约晚于原句结束 6.6 秒。详见 LIVE_TEST.md；不等于长时运行或真人听感通过。
...
| tts                                                           | 系统语音配音                                                                                                                                                                                                                                                                                              |
| optional_host_permissions `https://*/*`、`http://127.0.0.1/*` | 只在用户点击时申请**单一** origin：「授予访问权限」申请当前保存的 sub2api origin，「授予本机服务访问权限」申请 `http://127.0.0.1:<端口>`；服务地址或本地识别地址变化（含恢复默认设置清空地址）并保存成功后，移除不再被任何配置使用的旧 origin 的权限；manifest 必须声明较宽的可选范围才能支持任意用户域名 |
