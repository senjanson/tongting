...
3. 在扩展设置页「本地识别服务」中填写：
   - 服务地址：**必须**写 `http://127.0.0.1:8765`（端口改了就一起改），扩展只接受 `127.0.0.1`，不接受 `localhost`。原因：本服务只监听 IPv4 的 127.0.0.1；`localhost` 可能被解析到 IPv6 的 `::1`，而 `::1` 上的同一端口可能被其他进程占用，请求会发给错误的程序。
   - 配对令牌：粘贴上一步的输出。**不要把令牌贴到对话、文档或 issue 里。**
   - 点击「授予访问权限」，允许扩展访问 `http://127.0.0.1`。本服务**不返回任何 CORS 响应头**，扩展必须先拿到这个主机权限，才能从 offscreen 页面调用；否则浏览器会以跨域错误拦截请求。
...
| 扩展请求被跨域拦截 / 权限错误                 | 在设置页点击「授予访问权限」，允许 `http://127.0.0.1`。本服务不返回 CORS 头                                                                                                         |
